...
export default function EditArticle() {
    ...
    useEffect(() => {
        const fetchArticleData = async () => {
            try {
                if (!articleId) return;
                setIsLoading(true);
                const response = await axios.get(`http://localhost:5920/article/getarticlebyid/${articleId}`);
                if (response.status === 200) {
                    ...
                    setcategorieSelected(String(data.categoryId));
                    setUserId(data.userId); // Set userId from fetched data
                } else {
                    setErrorMessage("Failed to fetch article data.");
                }
            } catch (error: any) {
                console.error(error);
                setErrorMessage(error.response?.data?.message || "An unexpected error occurred while fetching article data.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const UpdateArticle = async (e: FormEvent<HTMLFormElement>) => {
        try {
            ...
            if (!title || !contenu || !langue || !summary || !categorieSelected || userId === null) { // Check userId
                setErrorMessage("Please fill in all required fields (Title, Content, Language, Summary, Category) and ensure user ID is available.");
                return;
            }

            setIsLoading(true);

            const updated_article = new FormData();
            updated_article.append("title", title);
            updated_article.append("slug", slug);
            updated_article.append("contenu", contenu);
            updated_article.append("langue", langue);
            updated_article.append("summary", summary);
            updated_article.append("published", String(published));
            updated_article.append("categoryId", categorieSelected);
            updated_article.append("userId", String(userId)); // Append userId
            
            if (publishedAt) {
                updated_article.append("publishedAt", new Date(publishedAt).toISOString());
            }

            if (selectedFile) {
                updated_article.append("image", selectedFile);
            }

            const response = await axios.put(`http://localhost:5920/article/updateArticle/${articleId}`, updated_article, {
            ...
            if (response.status === 200) {
                ...
                setErrorMessage(response.data.message || "Failed to update article. Please try again.");
            }
        } catch (error: any) {
            console.error(error);
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-lg text-gray-600">Update your article details</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                    <div className="px-8 py-6 bg-gradient-to-r from-blue-600 to-indigo-600">
                        ...
                        <p className="text-blue-100 mt-1">Fill in the information below to update your article</p>
                    </div>

                    <form onSubmit={UpdateArticle} className="px-8 py-8">
